...
type
  TfrShopVisual = class(TForm)
    ...
    sgQueue: TStringGrid;
    Label8: TLabel;
    lbCanceledCust: TLabel;
    Label9: TLabel;
    lbCanceledBuys: TLabel;
    shCashBack: TShape;
    shCash: TShape;
    Label10: TLabel;
    lbTotalBuys: TLabel;
...
begin
  ...
  lbCanceledCust.Caption := IntToStr(shp.CanceledCust);
  lbCanceledBuys.Caption := IntToStr(shp.CanceledBuys);
  lbTotalBuys.Caption := IntToStr(shp.TotalBuys);
  shCash.Width := Round(shp.CashStat.Mean * shCashBack.Width /
      shp.CashStat.Devices);
  ...
  if shp.CashStat.Running > 0 then
    lbCash.Caption := '(' + Chars(shp.CashStat.Running, '*') + ')'
  else
    lbCash.Caption := '';
