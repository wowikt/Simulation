...
begin
  ...
  // Отобразить танкеры, находящиеся у причала,
  //   отдельно еще не загруженные и загруженные, но ожидающие отчаливания
  lbLoading.Caption := Chars(prt.Berth.Busy - prt.Tug.Queue[1].Size, '*') +
      Chars(prt.Tug.Queue[1].Size, 'o');
  // Отобразить состояние буксира
