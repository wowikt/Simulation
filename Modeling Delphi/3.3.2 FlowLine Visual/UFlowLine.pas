...
begin
  ...
  if ((Queue1Size = 0) and (par.Stat1.Running = 0) and
      (par.Stat1.Blocked = 0)) or
      ((Queue1Size > 0) and (par.Queue1.Size < Queue1Size)) then
  begin
...
begin
  ...
  begin
    ...
    if ((Queue2Size = 0) and (par.Stat2.Running > 0)) or
        ((Queue2Size > 0) and (par.Queue2.Size >= Queue2Size)) then
    begin
      // Заблокировать процесс
      par.Stat1.StartBlock(SimTime);
      // Ожидать наличия места в очереди к второму рабочему месту
      while ((Queue2Size = 0) and (par.Stat2.Running > 0)) or
          ((Queue2Size > 0) and (par.Queue2.Size >= Queue2Size)) do
        Passivate;
      // Разблокировать
...
begin
  ...
  begin
    ...
    // Активировать первого работника
    par.Worker1.ActivateDelay(0);
