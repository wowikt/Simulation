...
begin
  ...
  if tmBank.Enabled then
    // Если имитация выполняется, остановить ее
    tmBank.Enabled := False
  else
  begin
    // Начало новой имитации
    MaxClientCount := StrToInt(edClientCount.Text);
    tmBank.Enabled := True;
    // Создать имитацию
    bnk.Free;
    bnk := TBankSimulation.Create;
    // Заполнить массив списков для отображения статистики
    Queue[0] := bnk.Queue;
...
begin
  // Таймер
  bnk.StopStat;
  // Отобразить состояние имитации
  lbQueue.Caption := Chars(bnk.Queue.Size, '*');
  if bnk.Cashman.Idle then
    lbCashman.Caption := ''
  else
    lbCashman.Caption := '(*)';
  ...
  // Если количество обслуженных изменилось
  if bnk.InBankHist.TotalCount > LastFinished then
  begin
    // Перерисовать гистограмму
    LastFinished := bnk.InBankHist.TotalCount;
    dgHistogram.Repaint;
  end;
  // Отобразить шкалу
  shCashBusy.Width := Round(shCashBack.Width * bnk.CashStat.Mean);
  // Если имитация завершена
  if bnk.Terminated then
    // Остановить таймер
    tmBank.Enabled := False
  else
    // Иначе - выполнить следующий фрагмент имитации
    RunSimulation(bnk);
end;

procedure TfrBank.FormCreate(Sender: TObject);
begin
  // Создание датчиков случайных чисел
  rndClient := TRandom.Create;
  rndCashman := TRandom.Create;
  // Настройка ширины столбцов табличных сеток
